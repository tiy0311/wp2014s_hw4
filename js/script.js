...
FB.getLoginStatus(function(response) {
  if (response.status === 'connected') {
    ...
    var IfLoggedInDiv=document.getElementById("if-logged-in");
        ...
        var CustomerizedTextDiv=document.getElementById("customerized-text");
        CustomerizedTextDiv.style.display="block";
        ...
        FB.api('/me/picture?width=150', function(response) {
            ...
            $("#preview").attr('src', my_picture_url);
        });
...
//以下為canvas的程式碼，基本上不需多動，依據comments修改即可
    ...
    function handleMouseMove(e){//滑鼠移動的event
      ...
      if(isDragging){ //當拖拉為True時
			...
			//canvas.width = profileIMG.width;   //設定canvas的大小需符合profileimg的大小 -- used in bonus
...
function PostImageToFacebook() {
    ...
    var fd = new FormData();
    fd.append("access_token", ACCESS_TOKEN);//請思考accesstoken要怎麼傳到這function內
    fd.append("source", blob);//輸入的照片
    fd.append("message", "這是HTML5 canvas和Facebook API結合教學QQQQQQ");//輸入的訊息
    try {
        $.ajax({
            url: "https://graph.facebook.com/me/photos?access_token=" + ACCESS_TOKEN,//GraphAPI Call
            type: "POST",
            data: fd,
            processData: false,
            contentType: false,
